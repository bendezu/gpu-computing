...
void blockMatrixMult() {
    ...
    auto secondtMatrixAsArray = generateIntArray(internalDim * cols2);
    ...
    timer.Start();
    tiledMultiplication(firstMatrixAsArray, secondtMatrixAsArray, resultAsArray);
    timer.Stop();
    cout << "GPU done in " << timer.Elapsed() << "ms" << endl << endl;

    cout << "Shared multiplication starts" << endl;
    timer.Start();
    sharedMultiplication(firstMatrixAsArray, secondtMatrixAsArray, resultAsArray);
    timer.Stop();
    cout << "GPU done in " << timer.Elapsed() << "ms" << endl << endl;
}
